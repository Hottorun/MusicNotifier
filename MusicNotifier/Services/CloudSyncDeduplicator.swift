...
enum CloudSyncDeduplicator {
    ...
    static func run(in context: ModelContext, force: Bool = false) {
        // The five merge passes below each fetch and materialize an entire
        ...
        if artistMerges + catalogMerges + releaseMerges + videoMerges + concertMerges > 0 {
            try? context.save()
            Log.v("[Dedup] merged \(artistMerges) artists (providerID), \(catalogMerges) artists (catalogID), \(releaseMerges) releases, \(videoMerges) videos, \(concertMerges) concerts")
    ...
    private static func currentCounts(in context: ModelContext) -> TableCounts {
        TableCounts(
            ...
            concerts: (try? context.fetchCount(FetchDescriptor<ConcertData>())) ?? 0
        )
    }
