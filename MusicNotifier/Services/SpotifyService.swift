...
struct SpotifyService {
    ...
    /// Empty unless something writes `spotifyClientID` at runtime — no client ID
    /// is bundled in v1. An empty value leaves `isConfigured` false and makes
    /// `authenticate()` throw `missingClientID`.
    var clientID: String {
        defaults.string(forKey: AppSettings.spotifyClientID)?
