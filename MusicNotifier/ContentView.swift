...
struct ContentView: View {
    ...
    // Every SwiftData save during a refresh (release upsert, videos)
    ...
    private var sidebar: some View {
        List(selection: sidebarSelectionBinding) {
            ...
            if enableVideosTab {
                Section {
                    sidebarRow(SidebarDestination.videos)
                } header: {
                    Text("Discover")
                }
            }
    ...
    private var tabSwitchGesture: some Gesture {
        DragGesture(minimumDistance: 60)
            .onEnded { value in
                ...
                let lastTabIndex: Int = enableVideosTab ? 3 : 2
                let target: Int
...
private enum SidebarDestination {
    case feed, upcoming, artists, videos
